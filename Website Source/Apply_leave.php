<?php
  ...
  if(isset($_POST["start_date"])) {
    ...
    $procedure_passed_params = array(
      ...
      array(&$procedure_params['type'], SQLSRV_PARAM_IN),
      array(&$procedure_params['output'], SQLSRV_PARAM_OUT)
    );

    // EXEC the procedure
    $sql = "EXEC Leave_Request_for_all @start = ?, @applicant = ?, @replacement = ?, @end_date = ?, @type = ?,@output = ?";
...
<body>
  ...
  <form action="Apply_leave.php" method="POST" class="form-group container">
    <div>
...
<input type='submit' name='submit3' value='Apply' class='register' />       <hr>

    </div>

    <a href="staff_member.php" class="btn">Cancel</a> 
